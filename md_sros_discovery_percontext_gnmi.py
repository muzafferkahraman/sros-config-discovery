    ...
    with gNMIclient(target=host,username="admin",password="admin",insecure=True) as gc:
        ...
        try:
            ...
            for card_item in main_array:
                ...
                data=gc.get(path=[f"/configure/card[slot-number={main['slot-number']}]/mda"],datatype="config")
                struct_array=data["notification"][0]["update"]
                for struct in struct_array:
                    # mdas=mda_struct["val"]
                    mdas=struct.get("val")
                    mdas = place_in_array(mdas)
                    if mdas:
                        for item in mdas:
                            json_snippet=Mda(
                            context="mda",
                ...
                data=gc.get(path=[f"/configure/card[slot-number={main['slot-number']}]/fp"],datatype="config")
                struct_array=data["notification"][0]["update"]
                for struct in struct_array:
                    # mdas=mda_struct["val"]
                    fps= struct.get("val")
                    fps = place_in_array(fps)
                    if fps:
                        for item in fps:
                            json_snippet=Fp(
                            context="fp",
